...
__gameRoot.GameRules = (() => {
  const defs = [
    ...
    {name:'Doppelgänger',icon:'🎭',type:'P',typeIcon:'📜',v:1,m:1,a:1,range:1,per:1},
  ...
  function defOf(p){
    ...
    const temp=(p.effects||[]).reduce((acc,e)=>{
      ...
      acc.v+=(Number(m.v)||0);acc.m+=(Number(m.m)||0);acc.a+=(Number(m.a)||0);acc.range+=(Number(m.range)||0);acc.per+=(Number(m.per)||0);
      return acc;
    },{v:0,m:0,a:0,range:0,per:0});
    return {...base,
  ...
  return Object.freeze({defs,skeletonDef,miniDef,lavaDef,baseBonuses,byName,rc,coord,inside,man,sameLine,blockedCells,isBlocked,neighbors,perceptionCells,defOf,attackCells,directWinner});
})();
...
__refRoot.GameReferee = class GameReferee {
  ...
  reset(){
    this.#s={
      ...
      history:{player:[],enemy:[]}, intel:{player:[],enemy:[]}, impact:{player:null,enemy:null}, perceptionHints:{player:[],enemy:[]},
  ...
  createClient(side){
    ...
    return Object.freeze({
      ...
      chooseDoppelCopy:(copyNew)=>self.#chooseDoppelCopy(side,copyNew)
    });
  }
  ...
  validateSetup(side,setup,bases){
    ...
    for(const x of setup){
      ...
      if(this.#R.isBlocked(x.coord))return this.#fail('Árvores bloqueiam a casa e não podem ser ocupadas.');
    ...
    for(const c of bases){
      ...
      if(this.#R.isBlocked(c))return this.#fail('Postos não podem ocupar uma casa com árvore.');
  ...
  startMultiplayerGame(playerSetup,playerBases,enemySetup,enemyBases){
    ...
    for(const [side,setup,bases] of [['player',playerSetup,playerBases],['enemy',enemySetup,enemyBases]]){
      for(const x of setup){const d=this.#R.byName[x.name];this.#s.pieces[side].push({id:(side==='player'?'p':'e')+this.#s.idSeq++,owner:side,name:d.name,hp:d.v,coord:x.coord,alive:true,activated:false,original:true,form:null,copied:null,mirrorCooldown:0,effects:[],bonusM:0,bonusV:0,bonusA:0,bonusRange:0,bonusRadarAdvanced:false,bonusRadarExpanded:false});}
  ...
  startGame(playerSetup,playerBases,difficulty='normal'){
    ...
    this.#s.pieces.player=playerSetup.map(x=>{
      const d=this.#R.byName[x.name];
      return {id:'p'+this.#s.idSeq++,owner:'player',name:d.name,hp:d.v,coord:x.coord,alive:true,activated:false,original:true,form:null,copied:null,mirrorCooldown:0,effects:[],bonusM:0,bonusV:0,bonusA:0,bonusRange:0,bonusRadarAdvanced:false,bonusRadarExpanded:false};
    });
    ...
    for(const d of chosen){
      ...
      used.add(c);
      this.#s.pieces.enemy.push({id:'e'+this.#s.idSeq++,owner:'enemy',name:d.name,hp:d.v,coord:c,alive:true,activated:false,original:true,form:null,copied:null,mirrorCooldown:0,effects:[],bonusM:0,bonusV:0,bonusA:0,bonusRange:0,bonusRadarAdvanced:false,bonusRadarExpanded:false});
    }
  ...
  #activationLimit(side){const own=this.#aliveActivationUnits(side),opp=this.#aliveActivationUnits(this.#other(side));return Math.min(own,opp+1);}
  #hasActivationLeft(side){const used=this.#s.roundActivations?.[side]||0;if(used>=this.#activationLimit(side))return false;return this.#pieces(side).some(p=>p.alive&&!p.activated);}
  ...
  #canShareCell(side,p,c){if(this.#R.isBlocked(c)||this.#baseAt(c))return false;const ps=this.#piecesAt(side,c).filter(x=>x.id!==p.id);if(!ps.length)return true;if(ps.length>=2)return false;return p.name==='Escudeiro'||ps.some(x=>x.name==='Escudeiro');}
  ...
  #originalDeaths(side){return this.#pieces(side).filter(p=>p.original&&!p.alive&&(p.name!=='Slime'||!this.#slimeLineageAlive(side,p.id))).length;}
  ...
  #addHistory(side,t){
    // O histórico registra acontecimentos relevantes; simples encerramentos de ativação ficam fora para reduzir ruído.
  ...
  #publicPiece(p,viewerSide=p.owner){
    ...
    return {id:p.id,name:p.name,displayName:d.name,icon:d.icon,type:d.type,typeIcon:d.typeIcon||'',hp:p.hp,maxHp:d.v,coord:p.coord,alive:p.alive,activated:p.activated,original:!!p.original,summonType:p.summonType||null,form:p.form||null,copied:p.copied||null,mirrorCooldown:p.mirrorCooldown||0,m:d.m,a:d.a,range:d.range,per:d.per,diag:!!d.diag,bonusM:p.bonusM||0,bonusV:p.bonusV||0,bonusA:p.bonusA||0,bonusRange:p.bonusRange||0,radarAdvanced:!!p.bonusRadarAdvanced,radarExpanded:!!p.bonusRadarExpanded,effects:(p.effects||[]).filter(e=>viewerSide===p.owner||e.public!==false).map(e=>({id:e.id||'',name:e.name||'Efeito temporário',icon:e.icon||'⏳',remaining:Math.max(0,Number(e.remaining)||0),kind:e.kind||'neutral',tick:e.tick||'round'}))};
  }

  #getView(side){
    const other=this.#other(side), act=this.#activation(side), visible=[];
    for(const e of this.#pieces(other)) if(e.alive&&this.#s.seer[side].has(e.coord)) visible.push(this.#publicPiece(e,side));
    ...
    return structuredClone({
      ...
      ownPieces:this.#pieces(side).map(p=>this.#publicPiece(p,side)),
      ...
      availablePieceIds:this.#pieces(side).filter(p=>p.alive&&this.#hasActivationLeft(side)&&!p.activated).map(p=>p.id),
      roundActivationsUsed:this.#s.roundActivations?.[side]||0, roundActivationLimit:this.#activationLimit(side)
    });
  }
  ...
  #selectPiece(side,id){
    ...
    if(!this.#hasActivationLeft(side))return this.#fail('Seu limite de ativações desta rodada já foi atingido.');
  ...
  #moveStep(side,to){
    ...
    return this.#ok(`${d.name}: ${a.moveRemaining===1?'resta':'restam'} ${a.moveRemaining} ${a.moveRemaining===1?'passo':'passos'}.`);
  }
  ...
  #finishMove(side){
    ...
    const detected=per>0&&(orthHits.length>0||(expanded&&diagHits.length>0));a.lastPerception=detected;
    const hintable=c=>!this.#R.isBlocked(c)&&!this.#pieceAt(side,c)&&!this.#baseAt(c);
    const hints=[];
    if(per>0){
  ...
  #startAbility(side){
    ...
    if(ab==='seer'){a.mode='seer';return this.#ok('👁️ Escolha uma casa principal + 3 casas adjacentes a ela (lado ou diagonal).',{ability:'seer'});}
    if(ab==='raise'){
  ...
  #useSeer(side,cells){
    ...
    const main=cells[0],adj=new Set(this.#R.neighbors(main,true));if(cells.slice(1).some(c=>!adj.has(c)))return this.#fail('As 3 casas extras precisam ser adjacentes à casa principal por lado ou diagonal.');
  ...
  #raiseAt(side,c){
    ...
    const summon={id:(side==='player'?'p':'e')+this.#s.idSeq++,owner:side,name:'Esqueleto',hp:1,coord:c,alive:true,activated:true,original:false,summonType:'skeleton',summonerId:p.id,form:null,copied:null,mirrorCooldown:0,effects:[],bonusM:0,bonusV:0,bonusA:0,bonusRange:0,bonusRadarAdvanced:false,bonusRadarExpanded:false};
    this.#pieces(side).push(summon);
    const hostile=this.#pieceAt(this.#other(side),c);
    if(hostile)this.#resolveSpawnConflict(side,summon,hostile,c);
    else this.#addHistory(side,`💀 ${this.#R.defOf(p).name} ergueu um Esqueleto.`);
  ...
  #placeMirror(side,c){
    ...
    const dist=this.#R.man(p.coord,c);if(c===p.coord||!this.#R.sameLine(p.coord,c)||dist<1||dist>2||this.#R.isBlocked(c)||this.#pieceAt(side,c)||this.#mirrorAt(c,side)||this.#baseAt(c))return this.#fail('Casa inválida para o Espelho.');
    ...
    p.mirrorCooldown=0;this.#addHistory(side,'🪞 Mago do Espelho criou um Espelho invisível para o adversário.');this.#addIntel(side,'🪞 Seu Espelho pode gerar falsa presença e reflete o primeiro ataque que acertá-lo.');a.mode=null;return this.#finishActivation(side);
  }
  ...
  #sabotageBase(side,baseId,bonusId,targetPieceId=null){
    ...
    let target=null;
    if(['radarAdvanced','radarExpanded','move','life','attack','range'].includes(bonusId)){
    ...
    if(target){
      ...
      if(bonusId==='radarAdvanced')target.bonusRadarAdvanced=true;
      if(bonusId==='radarExpanded')target.bonusRadarExpanded=true;
    }
  ...
  #kill(p){
    ...
    if(p.name==='Slime'&&p.original)this.#s.pendingSlimeSplits.push({owner:p.owner,coord:p.coord,lineageId:p.id,bonusM:p.bonusM||0,bonusV:p.bonusV||0,bonusA:p.bonusA||0,bonusRange:p.bonusRange||0,bonusPer:p.bonusPer||0,bonusRadarAdvanced:!!p.bonusRadarAdvanced,bonusRadarExpanded:!!p.bonusRadarExpanded,effects:(p.effects||[]).map(e=>structuredClone(e))});else this.#createCorpse(p);
    if(p.summonType==='miniSlime'&&p.slimeLineageId&&!this.#slimeLineageAlive(p.owner,p.slimeLineageId)){
      this.#addHistory(p.owner,'☠️ O último Mini-Slime caiu. A linhagem do Slime foi eliminada e agora conta como 1 perda.');
      this.#addHistory(this.#other(p.owner),'☠️ Uma eliminação inimiga foi confirmada.');
    }
  ...
  #hitAttack(side,attacker,to){
    ...
    if(!target){
      ...
      this.#addIntel(other,'💥 Ataque inimigo detectado: a casa atingida foi marcada no tabuleiro.');this.#s.impact[other]=to;return;
    }
    const targetSide=target.owner,stacked=this.#piecesAt(targetSide,to).length>1&&target.name==='Escudeiro';
    const before=this.#R.defOf(target).name,res=this.#damage(target,d.a),friendlyFire=targetSide===side,slimeSplit=target.name==='Slime'&&target.original&&res.dead;
    if(friendlyFire){
      this.#addHistory(side,slimeSplit?`🟢 Seu Slime foi rompido pelo seu ${d.name} e vai se dividir.`:res.dead?`⚠️ Seu ${before} foi eliminado pelo seu ${d.name}.`:res.transform?'🌋 Você atingiu seu Golem e o transformou em Golem de Lava.':`⚠️ Seu ${before} foi atingido pelo seu ${d.name}.`);
      if(stacked)this.#addIntel(side,'🛡️ Seu Escudeiro interceptou o ataque aliado e protegeu a outra peça.');
    }else{
      if(dist===1)this.#addHistory(side,slimeSplit?`🟢 Slime inimigo foi rompido por ${d.name} e se dividiu.`:res.dead?`☠️ ${before} inimigo eliminado por ${d.name}.`:res.transform?'🌋 Golem inimigo virou Golem de Lava.':`⚔️ ${before} inimigo foi atingido por ${d.name}.`);
      else this.#addHistory(side,slimeSplit?'🎯 O alvo distante foi atingido, mas nenhuma eliminação foi confirmada.':res.dead?'☠️ Um alvo distante foi eliminado.':'🎯 Um alvo distante foi atingido.');
      if(stacked)this.#addIntel(side,'🛡️ O Escudeiro inimigo interceptou o dano destinado à casa.');

      if(dist===1)this.#addHistory(other,slimeSplit?`🟢 Seu Slime foi rompido por ${d.name} e vai se dividir.`:res.dead?`☠️ Seu ${before} foi eliminado por ${d.name}.`:res.transform?'🌋 Seu Golem virou Golem de Lava.':`⚔️ Seu ${before} foi atingido por ${d.name}.`);
      else this.#addHistory(other,slimeSplit?'🟢 Seu Slime foi atingido por ataque distante e vai se dividir.':res.dead?`☠️ Seu ${before} foi eliminado por ataque distante.`:`🎯 Seu ${before} foi atingido por ataque distante.`);
  ...
  #resolveDirect(attackerSide,att,def,from,to){
    ...
    this.#resolveSlimeSplits();
    if(!res.dead){
      att.coord=from;const msg=res.transform?`🌋 ${this.#R.defOf(winner).name} venceu o Confronto contra ${before}; ${before} sofreu 1 e virou Golem de Lava.`:`⚔️ ${this.#R.defOf(winner).name} venceu o Confronto e causou 1 em ${before}, que sobreviveu.`;
    ...
    const slimeSplit=loser.name==='Slime'&&loser.original;
    const directMsg=slimeSplit?`🟢 Slime foi rompido por ${this.#R.defOf(winner).name} no Confronto Direto e se dividiu.`:`☠️ ${before} eliminado por ${this.#R.defOf(winner).name} em Confronto Direto.`;
    this.#addHistory(attackerSide,directMsg);
    this.#addHistory(defenderSide,directMsg);
    ...
    // O vencedor sempre escolhe onde termina. Em solo, se o vencedor for a IA,
    // a decisão volta ao Web Worker usando apenas a visão filtrada dela.
    ...
    return this.#ok('Confronto resolvido. O vencedor deve escolher onde termina.',{pendingCombat:true});
  }
  ...
  #tickPieceEffects(p,tick){
    ...
    for(const effect of p.effects){
      const e={...effect};
      if((e.tick||'round')===tick&&Number.isFinite(Number(e.remaining)))e.remaining=Number(e.remaining)-1;
  ...
  #finishActivation(side){
    const hadActivation=!!this.#activation(side),p=this.#activePiece(side);if(hadActivation)this.#s.roundActivations[side]=(this.#s.roundActivations?.[side]||0)+1;if(p&&p.alive){p.activated=true;this.#tickPieceEffects(p,'activation');}this.#s.activation[side]=null;this.#s.impact[side]=null;
  ...
  #advanceAfterActivation(side){
    ...
    if(!pLeft&&!eLeft){this.#s.round++;this.#tickRoundEffects();for(const p of this.#pieces('player'))p.activated=false;for(const p of this.#pieces('enemy'))p.activated=false;this.#s.roundActivations={player:0,enemy:0};this.#addHistory('player',`🔄 Rodada ${this.#s.round} começou.`);this.#addHistory('enemy',`🔄 Rodada ${this.#s.round} começou.`);this.#s.turn='player';return;}
  ...
  importState(raw){
    ...
    if(!this.#s.doppelChoice)this.#s.doppelChoice={player:null,enemy:null};
    if(!this.#s.roundActivations)this.#s.roundActivations={player:0,enemy:0};
    for(const side of ['player','enemy'])for(const p of this.#pieces(side))if(!Array.isArray(p.effects))p.effects=[];
  }
...
export class TriGameRoom {
  ...
  async runAI(){if(!this.started||!this.ai)return;let guard=0;while(guard++<80){const v=this.referee.client('C').getView();if(v.gameOver)break;if(!(v.pendingCombat||v.doppelChoice||v.turn==='C'))break;const act=this.ai.decide(v);if(!act||act.type==='wait')break;const res=applyTriAction(this.referee.client('C'),act);if(res?.ok)this.recordReplayAction('C',act);this.broadcastViews();await new Promise(r=>setTimeout(r,120));}}
