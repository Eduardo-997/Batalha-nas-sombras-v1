...
(() => {
  ...
  function drawArena(board,s){
    ...
    board.className='replay-arena';board.setAttribute('viewBox','-405 -355 810 710');board.innerHTML='';
    ...
    for(const side of ['A','B','C'])for(const t of s.traps?.[side]||[]){const [x,y]=centroid(t.coord),el=svgEl('text',{x:x+13,y:y+13,class:`replay-arena-marker side-${side}`});el.textContent=t.kind==='spot'?'🦉':'🪤';const ti=svgEl('title');ti.textContent=`Armadilha do Jogador ${side}`;el.appendChild(ti);board.appendChild(el);}
  ...
  function open(rawFrames,opts={}){
    ...
    function draw(){const f=frames[idx],s=f.state;range.value=idx;count.textContent=`${idx+1}/${frames.length}`;label.textContent=f.label||'Estado da partida';const active=arena?(s.turn?` · turno: Jogador ${s.turn}`:''):(s.turn?` · lado ativo: ${s.turn==='player'?'A':s.turn==='enemy'?'B':s.turn}`:'');meta.textContent=`Rodada ${s.round||1}${active}${s.gameOver?' · partida encerrada':''}`;if(arena)drawArena(board,s);else drawClassic(board,s);const hs=arena?arenaEvents(s):classicEvents(s);events.innerHTML=hs.length?hs.map(x=>`<div class="replay-event">${esc(x)}</div>`).join(''):'<div class="replay-muted">Sem evento registrado neste quadro.</div>';}
